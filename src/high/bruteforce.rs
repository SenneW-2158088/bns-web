...
> {
    ...
    let handle = tokio::spawn(async move {
        loop {
            let session = generate_sess_id(&client_clone).await.expect("expected a session");
            // Create headers with PHPSESSID cookie
            let mut headers = HeaderMap::new();
            // Combine both cookies into a single header value
            headers.insert(
                COOKIE,
                HeaderValue::from_str(&format!("PHPSESSID={}; security=high", &session.session_id))?,
            );

            // Make the request
            let payload = format!(
                "{}?username={}&password={}&user_token={}&Login=Login",
                URL, USERNAME, password, session.user_token
            );
